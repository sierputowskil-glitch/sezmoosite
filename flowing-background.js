(function () {
  ...
  var FSRC = [
    ...
    '  col = mix(col, smoke, smoothstep(0.55, 0.95, f) * 0.35 * r.y);',
    '  vec2 lightPos = vec2(0.08 + 0.05*sin(t*0.9), 0.18 + 0.04*cos(t*0.7));',
    '  float d = distance(uv, lightPos);',
    '  float glow = smoothstep(0.85, 0.0, d);',
    '  glow *= (0.35 + 0.65 * smoothstep(0.2, 0.85, f + 0.4*q.x));',
    '  col = mix(col, fog, glow * 0.9);',
    '  float d2 = distance(uv, vec2(0.02, 0.62));',
    '  float glow2 = smoothstep(0.6, 0.0, d2) * 0.25;',
    '  col = mix(col, fog * 0.7, glow2 * smoothstep(0.3, 0.8, f));',
    '  float core = smoothstep(u_glowRadius, 0.0, mdUv) * u_mAmt;',
    '  float ring = smoothstep(u_glowRadius + 0.09, u_glowRadius - 0.04, mdUv) * smoothstep(0.03, 0.10, mdUv) * u_mAmt;',
    '  col *= mix(1.0, u_shadowStrength, ring * 0.9);',
